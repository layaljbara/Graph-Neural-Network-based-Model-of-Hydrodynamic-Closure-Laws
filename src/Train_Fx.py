...
def train_model_four_sequential(
    ...
    results = []
    json_filename = f"{directory_name}Fx_det_results_{hidden_channels}channels_neighbours_{N_neigh}_optm.json"
...
if __name__ == "__main__":
    ...
    def prepare_data(df, df2):
        """
        Prepares dataset by applying transformations and moving it to the specified device.

        Parameters:
        df (pd.DataFrame): Data containing boundary neighbors.
        df2 (pd.DataFrame): Data containing angles.
    ...
    data_y_flipped = prepare_data(dfy, df2y)
    data_z_flipped = prepare_data(dfz, df2z)
    data_zy_flipped = prepare_data(dfzy, df2zy)
